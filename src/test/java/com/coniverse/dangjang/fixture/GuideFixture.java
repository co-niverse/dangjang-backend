...
public class GuideFixture {
	public static BloodSugarGuide 혈당_가이드_도큐먼트(String oauthId) {
		return BloodSugarGuide.builder().oauthId(oauthId).createdAt(LocalDate.now()).todayAlert(Alert.CAUTION).todayContent("가이드입니다").summary("요약입니다.").build();
	}

	public static SubGuide 혈당_서브_가이드(CommonCode type) {
		return SubGuide.builder().type(type).alert(Alert.CAUTION).content("가이드입니다").build();
	}
