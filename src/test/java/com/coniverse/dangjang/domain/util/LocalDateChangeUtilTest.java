...
class LocalDateChangeUtilTest {
	...
	void KST시간을_UTC에맞게_변환한다() {
		...
		assertThat(UTC변환날짜.getMinute()).isZero();
		assertThat(UTC변환날짜.getSecond()).isZero();
	}

	@Test
	void UTC시간을_KST에맞게_변환한다() {
		//given & when
		...
		assertThat(KST변환날짜.getHour()).isZero();
		assertThat(KST변환날짜.getMinute()).isZero();
		assertThat(KST변환날짜.getSecond()).isZero();
